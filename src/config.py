...
多轮验证说明：
  - RANDOM_SEEDS 列表中每个 seed 对应一轮实验
  - NUM_ROUNDS 自动等于 len(RANDOM_SEEDS)，不需要手动维护
  - 每轮用对应 seed 从全量数据集中重新采样 SAMPLE_SIZE 条，保证轮间数据独立
"""
import os
...
SAMPLE_SIZE     = 50   # 每轮随机采样条数，0 表示使用全部数据
FEVER_SPLIT     = "labelled_dev"  # 数据集划分：labelled_dev 或 labelled_test

VALID_LABELS = ["SUPPORTS", "REFUTES", "NOT ENOUGH INFO"]

# ================= 多轮验证配置 =================
# 设计思路：固定数据、重复推理，观察大模型输出的随机性对指标的影响
#   - RANDOM_SEED：控制数据采样，固定后每次拿到完全相同的 SAMPLE_SIZE 条数据
#   - NUM_ROUNDS ：对同一批数据重复跑 N 轮，每轮独立调用 API
#   - 多轮结果取均值 ± 标准差，反映模型在 temperature>0 时的输出稳定性
RANDOM_SEED = 123   # 采样随机种子，固定数据集，多轮共用
NUM_ROUNDS  = 3    # 重复验证轮数，改这里即可控制轮次
...
# 首次运行 build_bm25_index() 时自动创建，后续直接加载（多轮间复用，只构建一次）
